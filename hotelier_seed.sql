-- ================================================================
-- HOTELIER — Full Seed Data (MySQL 5.7 Compatible)
-- Database : 4748018_hotelier
-- ================================================================
-- This file:
--   1. Safely clears existing data (respects FK constraints)
--   2. Adds the roomNumber column if not yet present
--   3. Seeds 100 rooms across 5 floors (301-720)
--   4. Seeds 3 demo user accounts (guest / staff / admin)
--   5. Seeds the Guest profile for the demo guest account
-- ================================================================
-- Demo Login Credentials:
--   Role   | Email                  | Password
--   -------|------------------------|----------
--   guest  | guest@hotelier.com     | guest123
--   staff  | staff@hotelier.com     | staff123
--   admin  | admin@hotelier.com     | admin123
-- ================================================================
-- NOTE: Passwords are stored as PHP bcrypt hashes.
--       These hashes were generated with PASSWORD_DEFAULT (bcrypt cost 10).
-- ================================================================

SET FOREIGN_KEY_CHECKS = 0;

-- ----------------------------------------------------------------
-- STEP 1: Clear all tables (safe DELETE — respects FK off above)
-- ----------------------------------------------------------------
...
-- ----------------------------------------------------------------
-- STEP 3: Add roomNumber column (drop first if it already exists;
--         phpMyAdmin shows a harmless warning if column was missing)
-- ----------------------------------------------------------------
...
-- ================================================================
-- SEED: User accounts
-- Hashes verified to work with PHP password_verify()
-- guest123 hash:
--   php -r "echo password_hash('guest123', PASSWORD_DEFAULT);"
-- ================================================================
INSERT INTO `User` (username, password, role) VALUES
-- guest@hotelier.com  / guest123
('guest@hotelier.com',
 '$2y$10$u8QcI./l9zfV3cYR5Q4v7.4dPw0gH2zBQBV6.NKT5ELvDfFzTLiMi',
 'guest'),
-- staff@hotelier.com  / staff123
('staff@hotelier.com',
 '$2y$10$Xn3lPbHM5VPPgR7SSmBGW.mBF4.IlOCN0sGMBQbIR6bnH5q.W.OtC',
 'staff'),
-- admin@hotelier.com  / admin123
('admin@hotelier.com',
 '$2y$10$kIjx1V0kTDkdqPrB55vbwuWUSYiFHImxWI1o79z5UJ5FKxqJZBv6.',
 'admin');

-- ================================================================
-- SEED: Guest profile — links to User row 1 (guest@hotelier.com)
-- ================================================================
INSERT INTO `Guest` (guestID, fullName, email, phoneNumber) VALUES
(1, 'Guest User', 'guest@hotelier.com', '+60 12-345 6789');

-- ================================================================
-- SEED: 100 Rooms across 5 floors
-- Floor 3 & 4 : Standard Room  — RM 80/night
-- Floor 5     : Deluxe Room    — RM 150/night
-- Floor 6     : Executive Suite — RM 250/night
-- Floor 7     : Penthouse      — RM 500/night
-- ================================================================
